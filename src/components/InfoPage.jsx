...
function AboutBody() {
  return (
    <>
      <P>
        ColorVote는 대한민국 사람들이 가장 좋아하는 색을 투표로 모아,
        지역과 연령대에 따라 색 선호가 어떻게 달라지는지를 실시간으로 보여주는
        데이터 시각화 프로젝트입니다. 누구나 한 번의 투표로 참여할 수 있고,
      ...
      <P>
        공정한 집계를 위해 투표는 하루 1회로 제한되며, 중복 투표를 막기 위한
        최소한의 기술적 식별만 사용합니다.
      </P>
      ...
      <P>
        모든 수치는 실제 투표를 실시간으로 집계한 값이며, 특정 색이나 지역을
...
function PrivacyBody() {
  return (
    <>
      ...
      <Ul>
        <Li>투표 데이터: 선택한 색상, 지역, 연령대</Li>
        <Li>중복 투표 방지를 위한 최소한의 기술적 식별 정보</Li>
        <Li>브라우저 로컬 저장소(localStorage)에 저장되는 내 투표 기록</Li>
      </Ul>
      ...
      <P>
        투표 데이터는 통계 집계 목적으로 데이터베이스 호스팅 서비스(Supabase)에
      ...
      <P>
        ...
        투표 기록을 직접 삭제할 수 있습니다. 기타 문의 사항은 아래 연락처로 요청해
        주시기 바랍니다.
      </P>
